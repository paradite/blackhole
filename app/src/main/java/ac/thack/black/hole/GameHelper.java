...
public class GameHelper implements GoogleApiClient.ConnectionCallbacks,
    ...
    static final String TAG = "ac.thack.black.hole.GameHelper";
    ...
    /**
     * Construct a ac.thack.black.hole.GameHelper object, initially tied to the given Activity.
    ...
    void assertConfigured(String operation) {
        if (!mSetupDone) {
            String error = "ac.thack.black.hole.GameHelper error: Operation attempted without setup: "
                    + operation
    ...
    private void doApiOptionsPreCheck() {
        if (mGoogleApiClientBuilder != null) {
            String error = "ac.thack.black.hole.GameHelper: you cannot call set*ApiOptions after the client "
    ...
    public GoogleApiClient.Builder createApiClientBuilder() {
        if (mSetupDone) {
            String error = "ac.thack.black.hole.GameHelper: you called ac.thack.black.hole.GameHelper.createApiClientBuilder() after "
    ...
    /**
     * Performs setup on this ac.thack.black.hole.GameHelper object. Call this from the onCreate()
    ...
    public void setup(GameHelperListener listener) {
        if (mSetupDone) {
            String error = "ac.thack.black.hole.GameHelper: you cannot call ac.thack.black.hole.GameHelper.setup() more than once!";
            logError(error);
            throw new IllegalStateException(error);
        }
        mListener = listener;
    ...
    public void onStart(Activity act) {
        ...
        if (mConnectOnStart) {
            if (mGoogleApiClient.isConnected()) {
                Log.w(TAG,
                        "ac.thack.black.hole.GameHelper: client was already connected on onStart()");
            } else {
    ...
    public void enableDebugLog(boolean enabled, String tag) {
        Log.w(TAG, "ac.thack.black.hole.GameHelper.enableDebugLog(boolean,String) is deprecated. "
                + "Use ac.thack.black.hole.GameHelper.enableDebugLog(boolean)");
    ...
    public static void showFailureDialog(Activity activity, int actResp,
            int errorCode) {
        if (activity == null) {
            Log.e("ac.thack.black.hole.GameHelper", "*** No Activity. Can't show failure dialog!");
            return;
        }
        ...
        default:
            ...
            if (errorDialog == null) {
                // get fallback dialog
                Log.e("ac.thack.black.hole.GameHelper",
    ...
    void debugLog(String message) {
        if (mDebugLog) {
            Log.d(TAG, "ac.thack.black.hole.GameHelper: " + message);
        }
    }

    void logWarn(String message) {
        Log.w(TAG, "!!! ac.thack.black.hole.GameHelper WARNING: " + message);
    }

    void logError(String message) {
        Log.e(TAG, "*** ac.thack.black.hole.GameHelper ERROR: " + message);
    }
    ...
    // to a given state. This may be useful when using ac.thack.black.hole.GameHelper in a
    // non-standard
    // sign-in flow.
